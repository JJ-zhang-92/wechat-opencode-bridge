...
| 实现方式 | 为什么可行 | 为什么不需要改进 |
|----------|-----------|-----------------|
| `execSync` 获取会话列表 | 阻塞约 1 秒，单人可接受 | 异步会增加复杂度，没有实际收益 |
| `writeFileSync` 写日志 | SSD 写入延迟 0.1-1ms，无感知 | 异步批量写入只对高并发服务器有帮助 |
| 固定 5 秒 SSE 重连 | 简单可预测，个人使用够用 | 指数退避只在长时间宕机时有意义 |
| Map 无 TTL 清理 | 单用户最多 1-2 个活跃条目 | 残留条目占用约几字节，不值得增加复杂度 |
| 完整 NL prompt | 本地 Ollama，零成本 | Prompt 压缩只对 API 计费有意义 |

### 什么场景下需要改进

| 场景 | 需要改进什么 |
|------|-------------|
| 多用户部署 | 添加 Map TTL 清理、连接池、速率限制 |
| 云 API 计费（非本地 LLM） | 优化 prompt、添加响应缓存 |
| 网络存储 (NFS) 或 HDD | 改用异步日志写入 |
| 高可用要求 | 添加指数退避、熔断器 |
| 跨平台支持 | 用异步替代 `execSync` |

### 关键设计权衡

1. **简单优先于健壮** — `execSync` 是同步的但简单；单用户场景下简单优先
2. **本地优先于远程** — Ollama 意味着零成本，所以 prompt 长度无关紧要
3. **同步优先于异步** — 单用户场景下，阻塞操作完成得足够快
4. **正则优先，LLM 兜底** — 关键词覆盖 90% 的意图；LLM 只处理边界情况
